...
class OpenRouterSource(ModelSource):
    ...
    def discover_models(self) -> Iterable[Model]:
        ...
        for item in data.get("data", []):
            ...
            context_size = item.get("context_length") or provider_data.get(
                "context_length"
            )
        ...
        try:
            ...
            for item in data.get("models", []):
                ...
                yield Model(
                    ...
                    is_local=True,
                    connector_metadata={
                        "ollama": {"name": model_name, "base_url": self._base_url}
                    },
                    custom_metadata={
                        "size": item.get("size"),
    ...
    def discover_models(self) -> Iterable[Model]:
        ...
        try:
            response = requests.get(
                f"{self._base_url}/models", headers=headers, timeout=self._timeout
            )
            response.raise_for_status()
            data = response.json()
